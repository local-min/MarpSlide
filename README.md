...
- 原稿の情報充足度を診断し、不足があれば具体的な質問で補完（「このまま作成して」でスキップ可能）
...
3. 原稿の情報充足度を診断し、不足があれば質問で補完（スキップ可能）
4. 目的に応じたスライド構成を設計（12-20枚）
5. 各スライドに最適なレイアウトパターン（63種類から）を選択
6. 必要に応じて SVG 図表を自動生成
7. `slides/` ディレクトリにスライドファイルを出力
